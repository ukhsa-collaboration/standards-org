// this is borrowed from https://github.com/antfu/markdown-it-github-alerts/blob/main/src/index.ts
...
/**
 * Render GitHub Alerts as GDS notification banners plugin for markdown-it
 *
...
const markdownItGitHubAlerts = (md, options = {}) => {
  ...
  md.renderer.rules.alert_open = function (tokens, idx) {
    ...
    const suffix = classSuffix[type] ?? type
    const bannerClass = suffix === 'note' ? '' : ` govuk-notification-banner--${suffix}`

    return `<div class="govuk-notification-banner${bannerClass}" role="region" aria-labelledby="govuk-notification-banner-title" data-module="govuk-notification-banner">
  ...
  md.renderer.rules.alert_close = function (tokens, idx) {
    return `
  </div>
</div>`
  }
